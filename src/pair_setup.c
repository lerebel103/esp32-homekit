...
static int _subtlv_decrypt(enum hkdf_key_type htype,
                           ...
                           uint8_t *srp_key, uint8_t *device_msg, int device_msg_length, uint8_t **subtlv,
                           int *subtlv_length) {
...
static int _ios_device_signature_verify(void *iosdevices, uint8_t *srp_key, uint8_t *subtlv, int subtlv_length) {
    ...
    int verified = ed25519_verify((uint8_t *) &ios_device_ltpk->value, ios_device_ltpk->length,
                                  ...
                                  (uint8_t *) ios_device_info, ios_device_info_len);
...
static int
_acc_m6_subtlv(uint8_t *srp_key, char *acc_id, uint8_t *acc_ltk_public, uint8_t *acc_ltk_private, uint8_t **acc_subtlv,
               int *acc_subtlv_length) {
    ...
    ed25519_sign(acc_ltk_public, acc_ltk_private, (uint8_t*)acc_info, acc_info_len,
    ...
    sub_tlv_write_ptr += tlv_encode(HAP_TLV_TYPE_PUBLICKEY, ED25519_PUBLIC_KEY_LENGTH, acc_ltk_public,
                                    sub_tlv_write_ptr);
    ...
    *acc_subtlv = (uint8_t *) calloc(1, *acc_subtlv_length);
    ...
    chacha20_poly1305_encrypt(CHACHA20_POLY1305_TYPE_PS06, subtlv_key, NULL, 0, acc_plain_subtlv,
